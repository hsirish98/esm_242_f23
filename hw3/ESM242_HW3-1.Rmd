...
obj_func <- function(choice, a, b, c, p, delta, rotations){
   ...
   for(i in 2:(rotations+1)){
     state[i] = state[i-1] + choice[i]
     bens[i] = p*exp(a-(b/choice[i])) + ifelse(i != rotations+1, -c, 0)
   }
   
   pv = bens*(1/(1+delta))^state
   npv = sum(pv)
   return(-npv)
}
```
...
options=list("algorithm"="NLOPT_LN_COBYLA",xtol_rel=1e-8,maxeval=16000)
 ...
 out=nloptr(x0=1,eval_f=obj_func,
              lb=0,
              opts = options, 
              a=10, b=53.27, c=250, p=105, delta=0.05, rotations=1)
