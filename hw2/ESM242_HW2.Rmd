...
constraint <- function(E, stock_init, r, K, q, p, c, dsct, end_year){
  ...
  for(i in 2:(end_year+1)){
    stock[i] = stock[i-1] - q*E[i-1]*(stock[i-1])
...
results_q1 <- data.frame("Stocks" = stocks_q1, "Efforts"= efforts_q1) 
kable(results_q1)
```
...
results_q2 <- data.frame("Stocks" = stocks_q2, "Efforts"= efforts_q2) 
kable(results_q2)
```
...
```{r}
 q3_out=nloptr(x0=rep(1,times=50),eval_f=opt_func,
              ...
              lb=rep(0,times=50),
              opts = options, 
              stock_init = 0.1, r=0.9, K=1, q=0.01, p=500, c=0.01, dsct=0.05, end_year=49)
```
...
results_q3 <- data.frame("Stocks" = stocks_q3, "Efforts"= efforts_q3) 
kable(results_q3)
```
