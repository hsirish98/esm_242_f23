...
```{r}
obj_func <- function(q, R_init, end_year, delta, c0, c1){
...
R_constraint <- function(q, R_init = 1, end_year = 9, delta = 0.05, c0=0.8, c1=-0.1){
...
R_constraint(rep(0.1, times=10), R_init = 1, end_year = 9, delta = 0.05, c0=0.8, c1=-0.1)
...
options=list("algorithm"="NLOPT_LN_COBYLA",xtol_rel=1e-15,maxeval=16000)
 ...
 out=nloptr(x0=rep(0.1,times=10),eval_f=obj_func,
              ...
              ub=rep(1,times=10),
              opts = options, 
              R_init = 1, end_year = 9, delta = 0.05, c0=0.8, c1=-0.1)
...
obj_func_extra <- function(q, R_init, end_year, delta, c0, c1, lambda){
    ...
    npv=sum(pv) + (R_end*lambda)*((1/1+delta)^(end_year+1))
...
R_constraint2 <- function(q, R_init, end_year, delta = 0.05, c0=0.8, c1=-0.1, lambda=1.2){
  const <- vector(mode="numeric", length=length(q))
  R <- vector(mode="numeric", length=length(q))
...
options=list("algorithm"="NLOPT_LN_COBYLA",xtol_rel=1e-15,maxeval=16000)
 ...
 out_b =nloptr(x0=rep(0.1,times=10),eval_f=obj_func_extra,
              eval_g_ineq = R_constraint2,
              lb=rep(0,times=10),
              ub=rep(1,times=10),
              opts = options, 
 ...
 print(-out_b$objective)
 print(out_b$solution)
```
...
options=list("algorithm"="NLOPT_LN_COBYLA",xtol_rel=1e-15,maxeval=16000)
 ...
 out_c =nloptr(x0=rep(0.1,times=10),eval_f=obj_func_extra,
              eval_g_ineq = R_constraint2,
              lb=rep(0,times=10),
              ub=rep(1,times=10),
              opts = options, 
 ...
 print(-out_c$objective)
 print(out_c$solution)
...
```{r}
 ...
 out_e =nloptr(x0=rep(0.1,times=10),eval_f=obj_func,
              eval_g_ineq = R_constraint,
              lb=rep(0,times=10),
              ub=rep(1,times=10),
              opts = options, 
              R_init = 1, end_year = 9, delta = 0.05, c0=0.8, c1=0)

 print(-out_e$objective)
 print(out_e$solution)
